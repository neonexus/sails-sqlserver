...
module.exports = (function () {
  ...
  var adapter = {
    ...
    define: function (connection, collection, definition, cb) {
      ...
      adapter.connectConnection(connection, function __DEFINE__(err, uniqId) {
        ...
        var schemaName = getSchemaName(connection, collection)
        var tableName =  '[' + schemaName + ']' + '.[' + getTableName(collection) + ']';
    ...
    find: function (connection, collection, options, cb) {
      ...
      adapter.connectConnection(connection, function __FIND__(err, uniqId) {
        ...
        request.query(statement, function(err, recordset) {
          ...
          cb(null, utils.removeBlankSpaces(recordset));
        });
  ...
  function getSchemaName (connection, collection) {
    //This is the case when schema name is in map name itself
    if(collection.indexOf(".") != -1) return collection.split(".")[0];
  ...
  //gets the table name out of the collection
  function getTableName(collection){
    if(collection.indexOf(".") != -1) return collection.split(".")[1];
    return collection;
  }
